...
void merge(IndexEntry *&column, size_t &capacity, AvlTree T, vector<int64_t> &updates, int64_t posL, int64_t posH, int64_t _next) {
    int64_t remaining = posH - posL + 1;
    int64_t ins = posH;
    int64_t next = _next < 0 ? COLUMN_SIZE - 1 : _next;
    ...
    if (_next < 0) {
        COLUMN_SIZE = COLUMN_SIZE + remaining;
        while (COLUMN_SIZE >= capacity) {
            capacity *= 2;
    ...
    while (remaining > 0) {
        ...
        while (remaining > 0 && (updates[ins] >= node->Element)) {
            column[cur].m_key = updates[ins];
            cur--;
            ins--;
            remaining--;
        }
...
void merge_ripple(IndexEntry *&column,size_t &capacity, AvlTree T, vector<int64_t> &updates, int64_t posL, int64_t posH, int64_t low, int64_t high) {
    ...
    if (high >= node_to_swap_out->Element) {
        ...
        updates.erase(updates.begin() + posL, updates.begin() + posH + 1);
        return;
    }
    while (node_to_swap_out->offset + remaining + 1 >= COLUMN_SIZE) {
        column[COLUMN_SIZE].m_key = column[node_to_swap_out->offset+1].m_key ;
        column[node_to_swap_out->offset+1].m_key = updates[posH];
        posH--;
        COLUMN_SIZE++;
        remaining--;
...
//    IndexEntry stored_entries[remaining];
    ...
    for (auto i = 0; i < remaining; i++) {
        updates[posL + i] = stored_entries[i].m_key;
    }
    free(stored_entries);
}

void merge(IndexEntry *&column, size_t &capacity, SPSTTree T, vector<int64_t> &updates, int64_t posL, int64_t posH, int64_t _next) {
    int64_t remaining = posH - posL + 1;
    int64_t ins = posH;
    int64_t next = _next < 0 ? COLUMN_SIZE - 1 : _next;
    int64_t prevPos = next;
    ...
    int64_t w = 0;
    int64_t copy = 0;
    SPSTTree firstPiece = FindFirstPiece(T);
    SPSTTree node = NULL;
    ...
    while (remaining > 0) {
        ...
        node = FindNeighborsGTFinal(column[next].m_key, (SPSTTree) T);
        write = next + 1;
        cur = write + remaining - 1;
        while (remaining > 0 && (updates[ins] >= node->Element)) {
            column[cur].m_key = updates[ins];
...
void merge_ripple(IndexEntry *&column,size_t &capacity, SPSTTree T, vector<int64_t> &updates, int64_t posL, int64_t posH, int64_t low, int64_t high) {
    int64_t remaining = posH - posL + 1;
    SPSTTree lastPiece = FindLastPiece(T);
