//
// Created by cxk33 on 2021/4/24.
//

#ifndef SPST_NEW_SKIPLIST_WITH_RANK_H
#define SPST_NEW_SKIPLIST_WITH_RANK_H
#ifdef __cplusplus
extern "C"{
#endif

#include "../include/basestructs.h"
...
static struct skipnode *skipnode_new(int level, int64_t key, int64_t value)
{
...
skiplist_insert(struct skiplist *list, int64_t key, int64_t value)
{
    struct skipnode *nd;
    int rank[MAX_LEVEL];
...
static void skiplist_remove(struct skiplist *list, int64_t key)
{
    struct skipnode *node;
    int i = list->level - 1;
    struct sk_link *pos = &list->head[i];
...
static int64_t key_gte_min(int64_t key, struct range_spec *range)
{
    return range->maxex ? (key > range->max) : (key >= range->max);
}

static int64_t key_lte_max(int64_t key, struct range_spec *range)
{
    return range->minex ? (key < range->min) : (key <= range->min);
}

/* Returns if there is node key in range */
static int64_t key_in_range(struct skiplist *list, struct range_spec *range)
{
...
static int skiplist_key_rank(struct skiplist *list, int64_t key)
{
    int rank = 0;
    int i = list->level - 1;
    struct sk_link *pos = &list->head[i];
...
static struct skipnode *skiplist_search_by_key(struct skiplist *list, int64_t key)
{
    int i = list->level - 1;
    struct sk_link *pos = &list->head[i];
    struct sk_link *end = &list->head[i];
...
static IntPair createPair(struct skipnode* first, struct skipnode* second, int64_t limit){
    IntPair op = (IntPair)malloc(sizeof(struct int_pair));
    if (first && second) {
        if (first->value == 0)
            op->first = first->value;
        else
            op->first = first->value + 1;
...
/* search the node with specified key. */
static IntPair slFindNeighborsLT(int64_t key,struct skiplist *list, int64_t limit)
{
    int i = list->level - 1;
    struct sk_link *pos = &list->head[i];
    struct sk_link *end = &list->head[i];
    struct skipnode *node=0;
    struct skipnode *first=0, *second=0;
    for (; i >= 0; i--) {
        pos = pos->next;
        skiplist_foreach_forward(pos, end) {
            //没有看懂是如何通过这样复杂的转换得到这个节点的
            node = list_entry(pos, struct skipnode, link[i]);
            if (node->key >= key) {
                end = &node->link[i];
                break;
        ...
        //FIXME:不知道为什么错了，我觉得应该是跳表为空的原因，还要再看看
        if (i==0 && node && node->key >= key) {
            second = node;
            first = list_entry(pos->prev, struct skipnode, link[i]);
            //在最后创建Int_Pair
        }
        //在最后需要换新的一层，为什么是这样
        pos = end->prev;
    ...
    return createPair(first,second,limit);
}

/* search the node with specified key. */
static IntPair slFindNeighborsGTE(int64_t key, struct skiplist *list, int64_t limit)
{
    int i = list->level - 1;
    struct sk_link *pos = &list->head[i];
    struct sk_link *end = &list->head[i];
    ...
    for (; i >= 0; i--) {
        ...
        if (i==0 && node->key == key) {
            first = node;
            second = list_entry(pos->next, struct skipnode, link[i]);
            //在最后创建Int_Pair
        }
        //在最后需要换新的一层，为什么是这样
        pos = end->prev;
...
#endif //SPST_NEW_SKIPLIST_WITH_RANK_H
